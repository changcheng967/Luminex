...
// ============================================================
// FITTED PARAMETERS: FE_MG/FE_EG (eval_fitted.h, generated by
// nnue/openi_upload/gen_fitted.py) hold the coefficient of every eval
// feature in the solver index space (eval_feat.h). init_fitted_eval()
// pushes them into the runtime tables below, and every literal scalar
// in evaluate() reads FE directly — so applying a distillation fit is
...
static int PawnThreatMG[6] = {  0, 35, 40, 60, 90, 0 };
static int PawnThreatEG[6] = {  0, 40, 45, 70,100, 0 };

// Knight threats: minor piece attacking higher-value pieces
static int KnightThreatMG[6] = { 15,  5, 20, 45, 60, 0 };
static int KnightThreatEG[6] = { 20,  5, 25, 50, 65, 0 };

// Bishop threats: similar to knight (both are minors)
static int BishopThreatMG[6] = { 15, 20,  5, 45, 60, 0 };
static int BishopThreatEG[6] = { 20, 25,  5, 50, 65, 0 };

// Rook threats: rook threatening queen is strong
static int RookThreatMG[6] = { 10, 25, 25,  5, 50, 0 };
static int RookThreatEG[6] = { 12, 30, 30,  5, 55, 0 };

// Queen threats: queen is already powerful, threats are less "extra"
static int QueenThreatMG[6] = {  5, 15, 15, 15,  5, 0 };
static int QueenThreatEG[6] = {  5, 18, 18, 18,  5, 0 };
...
static int PassedMG[8] = { 0, -8,  0, 10, 25, 50, 100, 0 };
static int PassedEG[8] = { 0, 10, 20, 40, 70,120, 200, 0 };
...
static void evaluate_pawns(const Position& pos, int32_t& mg_out, int32_t& eg_out) {
    ...
    for (int c_idx = 0; c_idx < 2; ++c_idx) {
        ...
        while (pawns) {
            ...
            if (file_count[f] > 1) {
                mg += sign * FE_MG[feat::PAWN_DOUBLED];
                eg += sign * FE_EG[feat::PAWN_DOUBLED];
            }
            ...
            if (!left && !right) {
                mg += sign * FE_MG[feat::PAWN_ISOLATED];
                eg += sign * FE_EG[feat::PAWN_ISOLATED];
            }

            // Connected pawn: protected by another pawn (pawn chain)
            if (square_bb(sq) & supported_by_adj) {
                mg += sign * FE_MG[feat::PAWN_CONNECTED + r - 1];
                eg += sign * FE_EG[feat::PAWN_CONNECTED + r - 1];
            }
            ...
            // Pawn lever: can capture an adjacent enemy pawn (gaining space)
            {
                ...
                if (lever_targets) {
                    mg += sign * FE_MG[feat::PAWN_LEVER + r - 1];
                    // Central levers (c-f files) are more valuable
                    if (f >= FILE_C && f <= FILE_F) {
                        mg += sign * FE_MG[feat::PAWN_LEVER_C];
                    }
                }
            }
            ...
            // Backward pawn: can't safely advance (push square is attacked)
            {
                ...
                if (push < SQUARE_NONE && r >= RANK_2 && r <= RANK_5) {
                    ...
                    if (push_attacked) {
                        ...
                        if (!defended) {
                            mg += sign * FE_MG[feat::PAWN_BACKWARD];
                            eg += sign * FE_EG[feat::PAWN_BACKWARD];
                        }
                    }
                }
            }
...
Value evaluate(const Position& pos, bool tactical_only) {
    ...
    for (int c_idx = 0; c_idx < 2; ++c_idx) {
        ...
        while (pp) {
            ...
            if (connected) {
                ...
                mg_passer += n_connected * FE_MG[feat::PP_CONNECTED + r - 1];
                eg_passer += n_connected * FE_EG[feat::PP_CONNECTED + r - 1];
            }
            ...
            if (behind) {
                ...
                if (rook_behind) {
                    mg_passer += FE_MG[feat::PP_ROOK_BEHIND];
                    eg_passer += FE_EG[feat::PP_ROOK_BEHIND];
                }
            }
            ...
            if (enemy_behind) {
                ...
                if (enemy_rook_behind) {
                    mg_passer += FE_MG[feat::PP_ENEMY_ROOK];
                    eg_passer += FE_EG[feat::PP_ENEMY_ROOK];
                }
            }
            ...
            eg_passer += (their_kdist - our_kdist) * FE_EG[feat::PP_KING_PROX];
            ...
            if (their_kdist > our_kdist + (c == pos.side_to_move() ? 0 : 1)) {
                ...
                eg_passer += unreachable * (unreachable > 4 ? FE_EG[feat::PP_UNREACH_B]
                                                           : FE_EG[feat::PP_UNREACH_S]);
            }
        ...
        while (knights) {
            ...
            if (kr >= RANK_4 && kr <= RANK_6) {
                if (pawn_attacks_bb(c, our_pawns) & square_bb(sq)) {
                    ...
                    if (!(enemy_pawn_attacks & square_bb(sq))) {
                        mg_score += sign * FE_MG[feat::OUTPOST_N + kr - 3];
                        eg_score += sign * FE_EG[feat::OUTPOST_N + kr - 3];
                    }
                }
            }
            ...
            if (mob == 0) {
                mg_score += sign * FE_MG[feat::TRAPPED_N];
                eg_score += sign * FE_EG[feat::TRAPPED_N];
            }

            // Far from own king: harder to defend
            if (distance(sq, ksq_arr[c_idx]) > 3) {
                mg_score += sign * FE_MG[feat::FAR_N];
                eg_score += sign * FE_EG[feat::FAR_N];
            }
        ...
        while (bishops) {
            ...
            // Bishop with many same-color pawns: bad bishop penalty
            {
                ...
                mg_score += sign * FE_MG[feat::BSC + same_color_pawns];
                eg_score += sign * FE_EG[feat::BSC + same_color_pawns];
            }

            // Bishop on long diagonal (sees 2+ center squares)
            if (popcount(attacks & BB_CENTER) >= 2) {
                mg_score += sign * FE_MG[feat::BISH_LONG];
                eg_score += sign * FE_EG[feat::BISH_LONG];
            }
            ...
            // Bishop outpost
            {
                ...
                if (br >= RANK_4 && br <= RANK_6) {
                    if ((pawn_attacks_bb(c, our_pawns) & square_bb(sq)) &&
                        ...
                        mg_score += sign * FE_MG[feat::OUTPOST_B];
                        eg_score += sign * FE_EG[feat::OUTPOST_B];
                    }
                }
            }
            ...
            // Far bishop penalty
            if (distance(sq, ksq_arr[c_idx]) > 3) {
                mg_score += sign * FE_MG[feat::FAR_B];
                eg_score += sign * FE_EG[feat::FAR_B];
            }
        ...
        while (rooks) {
            ...
            if (!(pos.pieces(PAWN) & file_bb(f))) {
                mg_score += sign * FE_MG[feat::ROOK_OPEN];
                eg_score += sign * FE_EG[feat::ROOK_OPEN];
            } else if (!(our_pawns & file_bb(f))) {
                mg_score += sign * FE_MG[feat::ROOK_SEMI];
                eg_score += sign * FE_EG[feat::ROOK_SEMI];
            }

            // Rook on 7th rank
            Rank rr = relative_rank(c, rank_of(sq));
            if (rr == RANK_7) {
                mg_score += sign * FE_MG[feat::ROOK_7TH];
                eg_score += sign * FE_EG[feat::ROOK_7TH];
            }

            // Rook xray: same file as enemy queen
            if (file_bb(f) & pos.pieces(them, QUEEN)) {
                mg_score += sign * FE_MG[feat::ROOK_XRAY_Q];
                eg_score += sign * FE_EG[feat::ROOK_XRAY_Q];
            }

            // Far rook penalty
            if (distance(sq, ksq_arr[c_idx]) > 3) {
                mg_score += sign * FE_MG[feat::FAR_R];
                eg_score += sign * FE_EG[feat::FAR_R];
            }
            ...
            if (rr <= RANK_2) {
                ...
                if (king_between && mob <= 4) {
                    ...
                    if (has_castling) {
                        mg_score += sign * FE_MG[feat::ROOK_TRAP_C];
                        eg_score += sign * FE_EG[feat::ROOK_TRAP_C];
                    } else {
                        mg_score += sign * FE_MG[feat::ROOK_TRAP_NC];
                        eg_score += sign * FE_EG[feat::ROOK_TRAP_NC];
                    }
                }
            }
            ...
            // Rook on blocked file: own pawns ahead restrict activity
            {
                ...
                if (ahead_pawns) {
                    mg_score += sign * FE_MG[feat::ROOK_BLOCKED];
                    eg_score += sign * FE_EG[feat::ROOK_BLOCKED];
                }
            }
        ...
        while (queens) {
            ...
            // Far queen penalty
            if (distance(sq, ksq_arr[c_idx]) > 3) {
                mg_score += sign * FE_MG[feat::FAR_Q];
                eg_score += sign * FE_EG[feat::FAR_Q];
            }
        ...
        if (on_back) {
            for (int r = 1; r <= 3; ++r) {
                int weight = (r == 1) ? FE_MG[feat::SHIELD_R1]
                             : (r == 2) ? FE_MG[feat::SHIELD_R2]
                             : FE_MG[feat::SHIELD_R3];
            ...
            if (!(all_pawns & file_bb(kfile))) mg_score += sign * FE_MG[feat::OPEN_KFILE];
            if (kfile > FILE_A && !(all_pawns & file_bb(File(kfile - 1)))) mg_score += sign * FE_MG[feat::OPEN_ADJ];
            if (kfile < FILE_H && !(all_pawns & file_bb(File(kfile + 1)))) mg_score += sign * FE_MG[feat::OPEN_ADJ];
            ...
            for (int df = -1; df <= 1; ++df) {
                ...
                while (enemy_file_pawns) {
                    ...
                    if (pr >= RANK_4) {
                        mg_score += sign * (pr - 3) * FE_MG[feat::STORM];
                    }
                }
            }
        }
        ...
        if (castled) { mg_score += sign * FE_MG[feat::CASTLED]; eg_score += sign * FE_EG[feat::CASTLED]; }

        CastlingRight ks_cr = c == WHITE ? WHITE_KINGSIDE : BLACK_KINGSIDE;
        CastlingRight qs_cr = c == WHITE ? WHITE_QUEENSIDE : BLACK_QUEENSIDE;
        if (!pos.castling_allowed(c, ks_cr) && !pos.castling_allowed(c, qs_cr) && !castled) {
            mg_score += sign * FE_MG[feat::NOCASTLE];
            eg_score += sign * FE_EG[feat::NOCASTLE];
        }
        ...
        // -------------------------------------------------------
        {
            ...
            if (hanging_pawns) {
                mg_score += sign * FE_MG[feat::HANG_PAWN] * popcount(hanging_pawns);
                eg_score += sign * FE_EG[feat::HANG_PAWN] * popcount(hanging_pawns);
            }
            ...
            // Principle: a pinned piece can't move without exposing its king (Nimzowitsch)
            {
                ...
                if (enemy_pinned) {
                    ...
                    mg_score += sign * pin_count * FE_MG[feat::PINNED];
                    eg_score += sign * pin_count * FE_EG[feat::PINNED];
                }
            }
        }
    }
    ...
    if (!tactical_only) {
    {
        ...
        if (pawn_count[WHITE] > pawn_count[BLACK]) {
            mg_score += space[WHITE] * FE_MG[feat::SPACE_OWN] + space[BLACK] * FE_MG[feat::SPACE_OPP];
        } else if (pawn_count[BLACK] > pawn_count[WHITE]) {
            mg_score -= space[BLACK] * FE_MG[feat::SPACE_OWN] + space[WHITE] * FE_MG[feat::SPACE_OPP];
        }
    }
    ...
    for (int c_idx = 0; c_idx < 2; ++c_idx) {
        ...
        if (bishops >= 1) {
            mg_score += sign * (FE_MG[feat::IMBAL_CONST] + pawns * FE_MG[feat::IMBAL_PAWN]);
            eg_score += sign * (FE_EG[feat::IMBAL_CONST] + pawns * FE_EG[feat::IMBAL_PAWN]);
        }
    }
    ...
    // Bishop advantage with few pawns is already handled by imbalance term.
    {
        ...
        if (knight_adj > 0) {
            mg_score += w_knights * knight_adj * FE_MG[feat::KNIGHT_PAWN];
            mg_score -= b_knights * knight_adj * FE_MG[feat::KNIGHT_PAWN];
            eg_score += w_knights * knight_adj * FE_EG[feat::KNIGHT_PAWN];
            eg_score -= b_knights * knight_adj * FE_EG[feat::KNIGHT_PAWN];
        }
    }
    ...
    for (int c_idx = 0; c_idx < 2; ++c_idx) {
        ...
        if (attacker_count >= 2) {
            ...
            int flight_mg = (flight <= 1) ? FE_MG[feat::KS_FLIGHT1]
                          : (flight == 2) ? FE_MG[feat::KS_FLIGHT2] : 0;
            int flight_eg = (flight <= 1) ? FE_EG[feat::KS_FLIGHT1]
                          : (flight == 2) ? FE_EG[feat::KS_FLIGHT2] : 0;
...
// Push the fitted coefficients (FE_MG/FE_EG) into the runtime tables. The
// static initializers above are the pre-fit defaults; this runs first in
// init_evaluation(), so the tables always reflect eval_fitted.h.
static void init_fitted_eval() {
    using namespace feat;
    for (int pt = 0; pt < 5; ++pt) {
        PieceValueMG[pt] = FE_MG[MAT + pt];
        PieceValueEG[pt] = FE_EG[MAT + pt];
    }
    for (int pt = 0; pt < 6; ++pt)
        for (int s = 0; s < 64; ++s) {
            PST_MG_TABLE[WHITE][pt][s] = FE_MG[PST + pt * 64 + s];
            PST_EG_TABLE[WHITE][pt][s] = FE_EG[PST + pt * 64 + s];
        }
    for (int i = 0; i < 9;  ++i) { KnightMobilityMG[i] = FE_MG[MOB_N + i]; KnightMobilityEG[i] = FE_EG[MOB_N + i]; }
    for (int i = 0; i < 14; ++i) { BishopMobilityMG[i] = FE_MG[MOB_B + i]; BishopMobilityEG[i] = FE_EG[MOB_B + i]; }
    for (int i = 0; i < 15; ++i) { RookMobilityMG[i]   = FE_MG[MOB_R + i]; RookMobilityEG[i]   = FE_EG[MOB_R + i]; }
    for (int i = 0; i < 28; ++i) { QueenMobilityMG[i]  = FE_MG[MOB_Q + i]; QueenMobilityEG[i]  = FE_EG[MOB_Q + i]; }

    int* thr_mg[5] = { PawnThreatMG, KnightThreatMG, BishopThreatMG, RookThreatMG, QueenThreatMG };
    int* thr_eg[5] = { PawnThreatEG, KnightThreatEG, BishopThreatEG, RookThreatEG, QueenThreatEG };
    for (int a = 0; a < 5; ++a)
        for (int p = 0; p < 5; ++p) {
            thr_mg[a][p] = FE_MG[THR + a * 5 + p];
    ...
    g_eval_params.outpost_bishop_mg = FE_MG[OUTPOST_B];
    g_eval_params.outpost_bishop_eg = FE_EG[OUTPOST_B];
    g_eval_params.far_knight_mg = -FE_MG[FAR_N];
    g_eval_params.far_knight_eg = -FE_EG[FAR_N];
    g_eval_params.far_bishop_mg = -FE_MG[FAR_B];
    g_eval_params.far_bishop_eg = -FE_EG[FAR_B];
    g_eval_params.rook_open_mg = FE_MG[ROOK_OPEN];
    g_eval_params.rook_open_eg = FE_EG[ROOK_OPEN];
    g_eval_params.rook_semi_open_mg = FE_MG[ROOK_SEMI];
    g_eval_params.rook_semi_open_eg = FE_EG[ROOK_SEMI];
    g_eval_params.rook_7th_mg = FE_MG[ROOK_7TH];
    g_eval_params.rook_7th_eg = FE_EG[ROOK_7TH];
    g_eval_params.pawn_shield_knight = FE_MG[SHIELD_R1];
    g_eval_params.pawn_shield_center = FE_MG[SHIELD_R2];
    g_eval_params.pawn_shield_rook = FE_MG[SHIELD_R3];
    g_eval_params.open_file_penalty_mg = -FE_MG[OPEN_KFILE];
    g_eval_params.open_file_penalty_eg = -FE_MG[OPEN_ADJ];
    g_eval_params.pawn_storm = -FE_MG[STORM];
    g_eval_params.hanging_pawn_mg = FE_MG[HANG_PAWN];
    g_eval_params.hanging_pawn_eg = FE_EG[HANG_PAWN];
    g_eval_params.bishop_pair_mg = FE_MG[BISHOP_PAIR];
    g_eval_params.bishop_pair_eg = FE_EG[BISHOP_PAIR];
}
